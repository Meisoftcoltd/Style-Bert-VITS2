...
def run():
    ...
    parser.add_argument(
        ...
        "--model",
        type=str,
        help="Ruta de la carpeta del conjunto de datos. Tenga en cuenta que los datos ya no se colocan de forma predeterminada en la carpeta /logs. Si necesita configurar mediante línea de comandos, declare la ruta relativa al directorio raíz",
        default=config.dataset_path,
    )
    parser.add_argument(
        "--assets_root",
        type=str,
        help="Directorio raíz de los activos del modelo necesarios para la inferencia.",
        default=config.assets_root,
    )
    parser.add_argument(
        "--skip_default_style",
        action="store_true",
        help="Omitir guardar la configuración de estilo predeterminada y el vector medio.",
    )
    parser.add_argument(
        "--no_progress_bar",
        action="store_true",
        help="No mostrar la barra de progreso durante el entrenamiento.",
    )
    parser.add_argument(
        "--speedup",
        action="store_true",
        help="Acelerar el entrenamiento deshabilitando el registro y la evaluación.",
    )
    parser.add_argument(
        "--repo_id",
        help="ID del repositorio del modelo de Huggingface para hacer una copia de seguridad del modelo.",
        default=None,
    )
    parser.add_argument(
        "--not_use_custom_batch_sampler",
        help="No usar el muestreador de lotes personalizado para el entrenamiento, que se usaba en la versión < 2.5",
        action="store_true",
    )
    ...
    for env_name, env_value in envs.items():
        if env_name not in os.environ.keys():
            logger.info(f"Cargando configuración desde config {env_value!s}")
    ...
    # Comparar si las rutas son iguales
    ...
    if args.repo_id is not None:
        ...
        try:
            ...
            logger.error(e)
            logger.error(
                f"Error al subir archivos al repositorio {hps.repo_id}. Por favor verifique si el repositorio existe y ha iniciado sesión usando `huggingface-cli login`."
            )
            raise e
    ...
    # use_jp_extra=True is used to enable the single-bert-feature mode which we adapted for ES in data_utils.py
    ...
    if not args.not_use_custom_batch_sampler:
        ...
        train_loader = DataLoader(
            train_dataset,
            # Intentando reducir el consumo de memoria configurando num_workers a 1
            ...
            # Comentado para intentar reducir consumo de memoria
            # prefetch_factor=6,
        )
    else:
        ...
        logger.info("Usando DistributedLengthGroupedSampler para el entrenamiento.")
    ...
    if hps.model.use_noise_scaled_mas is True:
        logger.info("Usando noise scaled MAS para VITS2")
        mas_noise_scale_initial = 0.01
        noise_scale_delta = 2e-6
    else:
        logger.info("Usando MAS normal para VITS1")
        mas_noise_scale_initial = 0.0
        noise_scale_delta = 0.0
    if hps.model.use_duration_discriminator is True:
        logger.info("Usando discriminador de duración para VITS2")
    ...
    if hps.model.use_spk_conditioned_encoder is True:
        if hps.data.n_speakers == 0:
            raise ValueError(
                "n_speakers debe ser > 0 cuando se usa spk conditioned encoder para entrenar modelo multi-speaker"
            )
    else:
        logger.info("Usando encoder normal para VITS1")
    ...
    net_g = SynthesizerTrn(
        ...
        # Pasar todos los valores bajo hps.model como argumentos
    ...
    # Check for freeze_ES_bert or freeze_JP_bert (for compatibility if user didn't change config)
    if getattr(hps.train, "freeze_ES_bert", False) or getattr(hps.train, "freeze_JP_bert", False):
        logger.info("¡¡¡ Congelando encoder bert (ES) !!!")
        for param in net_g.enc_p.bert_proj.parameters():
            param.requires_grad = False
    if getattr(hps.train, "freeze_style", False):
        logger.info("¡¡¡ Congelando encoder de estilo !!!")
        for param in net_g.enc_p.style_proj.parameters():
            param.requires_grad = False

    if getattr(hps.train, "freeze_decoder", False):
        logger.info("¡¡¡ Congelando decoder !!!")
    ...
    if utils.is_resuming(model_dir):
        ...
        if net_wd is not None:
            try:
                ...
                if not optim_wd.param_groups[0].get("initial_lr"):
                    optim_wd.param_groups[0]["initial_lr"] = wd_resume_lr
                logger.info("Inicializando wavlm")
        ...
        try:
            ...
            logger.info(
                f"******************Modelo encontrado. Epoca actual es {epoch_str}, paso global es {global_step}*********************"
            )
        except Exception as e:
            logger.warning(e)
            logger.warning(
                "Parece que no estás usando los modelos preentrenados, así que entrenaremos desde cero."
            )
            epoch_str = 1
            global_step = 0
    else:
        try:
            ...
            logger.info("Modelos preentrenados cargados.")
        except Exception as e:
            logger.warning(e)
            logger.warning(
                "Parece que no estás usando los modelos preentrenados, así que entrenaremos desde cero."
...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        if pbar is not None:
            pbar.set_description(
                f"Epoca {epoch}({100.0 * batch_idx / len(train_loader):.0f}%)/{hps.train.epochs}"
            )
            pbar.update()
    ...
    if pbar is None and rank == 0:
        logger.info(f"====> Epoca: {epoch}, paso: {global_step}")
